...
{
	printk( KERN_INFO "Adding an entry to proc FS - /proc/jiffies\n");

	// Create a /proc entry name jiffies
...
{
	// Remove the /proc/jiffies entry created in sarg()
...
ssize_t my_proc_read( struct file *file, char __user *user_buffer, size_t count, loff_t *pos)
{
	...
	completed = 1;
	bytes_written = sprintf(kernel_buffer, "Number of timer interrupts (jiffies): %lu\n", jiffies);
...
MODULE_DESCRIPTION( "Returns a buffer to user (storing value of jiffies) whenever /proc/jiffies file is read" );
